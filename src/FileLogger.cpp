...
FileLogger::FileLogger(const LoggerConfig& cfg) : config_(cfg), platform_(detect_platform()) {
    ...
    if (config_.toFile) {
        ...
        // 若包含父路径则自动创建目录，提升鲁棒性
        std::filesystem::path path = std::filesystem::u8path(config_.filePath);
        if (path.has_parent_path() && !path.parent_path().empty()) {
            std::error_code ec;
            std::filesystem::create_directories(path.parent_path(), ec);
            if (ec) {
                throw std::runtime_error("创建日志目录失败: " + ec.message());
            }
        }
        ...
        current_size_ = safe_file_size(path);
...
void FileLogger::rotate_files() const {
    ...
    std::filesystem::path base = std::filesystem::u8path(config_.filePath);
    if (file_.is_open()) {
        file_.close();
    }
    ...
    if (config_.maxBackupFiles > 0) {
        for (std::size_t i = config_.maxBackupFiles; i > 0; --i) {
            std::filesystem::path target = base;
            target += "." + std::to_string(i);
            std::filesystem::path source =
                (i == 1) ? base : std::filesystem::path(base.string() + "." + std::to_string(i - 1));
            std::error_code ec_rm, ec_mv;
            std::filesystem::remove(target, ec_rm); // 删除已有备份
            if (std::filesystem::exists(source)) {
                std::filesystem::rename(source, target, ec_mv);
            }
        }
    } else {
        std::error_code ec;
    ...
    // 重新打开主文件，重置大小与分割线状态
    file_.open(base, std::ios::out | std::ios::trunc);
    current_size_ = 0;
